...
// Parametrize it? (needed for cache FI)
#define ADDR_BITS	32

using StuckAtMap = std::map<uint64_t, std::vector<vp::FIRequest *>>;

// Workaround for priority queue
...
private:
    vp::FIRequest *assemble_request(int cmd, int kind, int target_type, int target, 
        uint64_t addr, uint64_t bit, uint64_t duration, uint8_t is_high, uint64_t delay, 
    ...
    : vp::Component(config)
{
    ...
    long long int cutoff_cycle = get_js_config()->get_int("max_cycle");
    if (cutoff_cycle > 0)
    {
        vp::FIRequest *fir = new vp::FIRequest();
        fir->cmd = vp::FICommand::STOP_RIGHT_NOW;
        fir->target_cycle = (uint64_t) cutoff_cycle;
        ...
        if (file.is_open()) 
        {
                ...
                >> bit >> duration >> is_high >> delay >> size >> id) 
            {
                vp::FIRequest *fir = this->assemble_request((int) cmd, (int) kind, (int) target_type, 
                    (int) target, (uint64_t) addr, (uint64_t ) bit, (uint64_t) duration, 
...
vp::FIRequest *FIC::assemble_request(int cmd, int kind, int target_type, 
    int target, uint64_t addr, uint64_t bit, uint64_t duration, uint8_t is_high, 
...
void FIC::inject_fault(vp::FIRequest *fir)
{
    if (fir->fault_type == vp::FIFaultType::BITFLIP) 
    {
        ...
        else if (fir->target_type == vp::FITargetType::REGISTER)
        {
            ...
            else if (reg >= this->regfile_nbs[regfile_id])
            {
                fprintf(stderr, "Register ID %lu outside of the regfile scope!\n", reg);
            }
            else
            {
                if (this->regfile_types[regfile_id] == VP_FI_VREG)
                {
                    size_t bytes_per_reg = this->regfile_sizes[regfile_id] / 8;	

                    uint8_t *vregs = (uint8_t *) this->regfile_ptrs[regfile_id];

                    uint8_t *vreg = vregs + reg * bytes_per_reg;

                    uint64_t byte = fir->bit / 8;
                    uint64_t bit = fir->bit % 8;
    ...
    else if (fir->fault_type == vp::FIFaultType::STUCK_AT)
    {
        if (fir->target_type == vp::FITargetType::MEMORY)
        {
            ...
            else
            {
                ...
                // Adjust the stuck_at map at target
                stuck_at_map[fir->addr].push_back(tf);

                // And do the immediate modification
                uint8_t mask = (uint8_t) (1u << fir->bit);

                if (fir->is_high)
                {
                    this->memory_ptrs[mem_id][fir->addr] |= mask;
                }
                else
                {
                    this->memory_ptrs[mem_id][fir->addr] &= ~mask;
...
void FIC::handle_event(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    while (!_this->fault_queue.empty())
    {
        ...
        {
            if (front->cmd == vp::FICommand::STOP_RIGHT_NOW)
            {
                fprintf(stderr, "FATAL! Reached cutoff cycle %lu!\n", front->target_cycle);
                _this->time.get_engine()->quit(6666);
            }
    ...
    if (!this->hash_requests.empty())
    {
        ...
        while (!this->hash_requests.empty())
        {
            ...
            else
            {
                ...
                {
                    fprintf(stderr, "Memory region 0x%x-0x%x to hash is out-of-bounds for target=%d!\n", 
                        addr, addr + size, mem_id);
                }

                uint64_t hash = 0; 
                if (addr == 0 && size == 0) 
                {
                    hash = hash_data(this->memory_ptrs[mem_id], this->memory_sizes[mem_id]); 
                }
                else 
                {
                    uint8_t *start = this->memory_ptrs[mem_id];
                    uint8_t *to_hash = &(start[addr]);
                    hash = hash_data(to_hash, size);
                }
...
uint64_t FIC::hash_data(uint8_t *start, uint64_t len)
{
    ...
    for (uint64_t i = 0; i < len; i++) {
        hash ^= (uint64_t) start[i];
        hash += i;	// Add this for no reason
